...
class FmsSentController extends GetxController {
  bool isLoading = true;
  String outboxType = kSelf;

  ReportingEmp? selectedReportingEmp;

  List<FmsMail> fmsOutboxList = [];
  ...
  void onInit() async{
    ...
    // calling get fms outbox list method
    getFmsOutboxList(id: _cpfNumber);
  }
  ...
  void onReady() {
    ...
    // GailConnectServices.to.hitCountApi(activity: kFMSOutboxScreen);
  }

  getFmsOutboxList({required String id}) async {
    isLoading = true;
    update([kFmsSent]);

    // calling get fms mail list api method
    fmsOutboxList = await GailConnectServices.to
        .getFmsMailListApi(id: id, api: kFmsOutboxApi);
  ...
  onInboxTypeSelected(String? _selectedValue) async{
    outboxType = _selectedValue!;
    update([kFmsSent]);
  ...
  onSubOrdinateSelected(ReportingEmp? _selectedReportingEmp) {
    ...
    // calling get fms outbox list method
    getFmsOutboxList(id: _selectedReportingEmp!.empNo!);
  }
}
