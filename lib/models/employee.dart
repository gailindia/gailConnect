...
class Employee {
  ...
  factory Employee.fromJson(Map<String, dynamic> _employeeJson) => Employee(
        empNo:
            _employeeJson[kJsonEmpNoCaps] ?? _employeeJson[kJsonEmpNoFullCaps],
        empName: _employeeJson[kJsonEmpNameCaps] ??
            _employeeJson[kJsonEmpNameFullCaps],
        designation: _employeeJson[kJsonDesignation] ??
            _employeeJson[kJsonDesignationCaps],
        department: _employeeJson[kJsonDepartment] ??
            _employeeJson[kJsonDepartmentFullCaps],
        directorate:
            _employeeJson[kJsonDirectorate] ?? _employeeJson[kJsonDirectorate],
        section: _employeeJson[kJsonSection] ?? _employeeJson[kJsonSection],
        DateOfJoining: _employeeJson[kColumnDateOfJoining] ??
            _employeeJson[
                kColumnDateOfJoining], // _employeeJson[kColumnDateOfJoining]
        location: _employeeJson[kJsonLocationCaps] ??
            _employeeJson[kJsonLocationFullCaps],
        ...
        grade:
            _employeeJson[kJsonGradeCaps] ?? _employeeJson[kJsonGradeFullCaps],
        gradeValue: _gradeValues[
            _employeeJson[kJsonGradeCaps] ?? _employeeJson[kJsonGradeFullCaps]],
  ...
  factory Employee.fromDbMap(Map<String, dynamic> _employeeJson) => Employee(
        ...
        empName: _employeeJson[kColumnEmpName]
...
class EmployeeDb {
  ...
  static Future<Employee?> getLoggedInEmployeeFromDb() async {
    ...
    try {
      ...
      final String _query =
          "SELECT * FROM $kTableEmployees WHERE $kColumnEmpNo = '$_cpfNumber'";
      final _result = await _db!.rawQuery(_query);

      if (_result.isNotEmpty) {
        final List<Employee> _employeeList = _result
            .map<Employee>((_employeeJson) => Employee.fromDbMap(_employeeJson))
            .toList();
  ...
  static Future<List<Employee>> getGroupEmployees(
      {required String groupMembers}) async {
    try {
      final _db = await DbProvider.db.database;
      final String _groupMembers = groupMembers
          .split(',')
          .map((_member) => "'${_member.padLeft(8, '0')}'")
          .join(',');

      final String _query =
          "SELECT * FROM $kTableEmployees WHERE $kColumnEmpNo IN ($_groupMembers)";
      final _result = await _db!.rawQuery(_query);
  ...
  static Future<List<Employee>> getEmployeesListBySearchQueryFromDb(
  ...
  static Future<List<Employee>> getEmployeesListBySearchQuerywithfilterFromDb({
    ...
    try {
      ...
      if (section != '') {
        String findSection =
            "select $kSection from $kTableSectionMaster where $kSectionDes = '$section'";
        final _result = await _db!.rawQuery(findSection);
        final subDepartmentList =
            _result.map<String>((s) => s['SECTION'].toString());
        section = subDepartmentList.first;
      }
      if (empGrade == 'Executives') {
        _whereClause =
            "where $kColumnLocation = (case when '$location' = '' then $kColumnLocation else '$location' end) and $kColumnGrade between 'E0' and 'E9' and $kColumnDepartment = (case when '$department' = '' then $kColumnDepartment else '$department' end) and $kColumnSDirectorate = (case when '$directorate' = '' then $kColumnSDirectorate else '$directorate' end) and $kColumnSection = (case when '$section' = '' then $kColumnSection else '$section' end) AND $kColumnEmpName LIKE '%$_searchQuery%' "
            "OR $kColumnEmpNo LIKE '%$_searchQuery%' "
            "OR $kColumnTelNo LIKE '%$_searchQuery%' "
            "OR $kColumnMobileNo LIKE '%$_searchQuery%' "
            "ORDER BY $kColumnEmpName ASC;";
        String q =
            "select count(*) from $kTableEmployees where $kColumnGrade between 'E0' AND 'E9' ";
        final _result1 = await _db!.rawQuery(q);
        print("count from executive :: $_result1");
      } else if (empGrade == 'Non-Executives') {
        _whereClause =
            "where $kColumnLocation = (case when '$location' = '' then $kColumnLocation else '$location' end) and $kColumnGrade between 'S0' and 'S7' and $kColumnDepartment = (case when '$department' = '' then $kColumnDepartment else '$department' end) and $kColumnSDirectorate = (case when '$directorate' = '' then $kColumnSDirectorate else '$directorate' end) and $kColumnSection = (case when '$section' = '' then $kColumnSection else '$section' end) AND $kColumnEmpName LIKE '%$_searchQuery%' "
            "OR $kColumnEmpNo LIKE '%$_searchQuery%' "
            "OR $kColumnTelNo LIKE '%$_searchQuery%' "
            "OR $kColumnMobileNo LIKE '%$_searchQuery%' "
            "ORDER BY $kColumnEmpName ASC;";
        String q =
            "select count(*) from $kTableEmployees where $kColumnGrade between 'S0' AND 'S7' ";
        final _result1 = await _db!.rawQuery(q);
  ...
  static Future<List<Employee>> getEmployeesListFromDbwithfiltersearch({
    ...
    final String? _cpfNumber =
        await pref.getString("cpfNumber", isEncrypted: true);
    final String? _contactListBy =
        await pref.getString("contactListBy") ?? kMyLocationAndDepartment;
    final String? _sortListBy =
        await pref.getString("sortContactListBy") ?? kAlphabetWiseSorting;
    print(
        "jhge e gew f fwkgyfk wkuyg ffku fkgu fu fku ff fiewuifbds fsjvfd   ${_contactListBy}   ${_sortListBy}");

    try {
      String _whereClause = "", _sortingOrder = '';
      debugPrint(
          'all emp value is: $allEmployees && is filter selected value is: $isFilterSelected');

      final _db = await DbProvider.db.database;
      final String? _location =
          MainDashController.to.loggedInEmployee?.location;
      final String? _department =
          MainDashController.to.loggedInEmployee?.department;

      if (forBirthdayList) {
        final String _currentDateTime = DateTime.now()
            .formatDateTime(newDateTimeFormat: kDateOfBirthFormat);
        ...
        if (allEmployees) {
          ...
          if (section != '') {
            ...
            if (searchQuery == "") {
              ...
              findSection =
                  "select $kSection from $kTableSectionMaster where $kSectionDes = '$section' AND $kColumnEmpName LIKE '%$searchQuery%' "
          ...
          if (_contactListBy == kMyLocationAndDepartment) {
            _whereClause =
                "WHERE $kColumnDepartment = '$_department' AND $kColumnLocation = '$_location'  AND $kColumnEmpName LIKE '%$searchQuery%' "
                "OR $kColumnEmpNo LIKE '%$searchQuery%' "
                "OR $kColumnTelNo LIKE '%$searchQuery%' "
                "OR $kColumnMobileNo LIKE '%$searchQuery%' "
                "ORDER BY $kColumnEmpName ASC";
            ...
            _whereClause =
                "WHERE $kColumnLocation = '$_location' AND $kColumnEmpName LIKE '%$searchQuery%' "
                "OR $kColumnEmpNo LIKE '%$searchQuery%' "
                "OR $kColumnTelNo LIKE '%$searchQuery%' "
                "OR $kColumnMobileNo LIKE '%$searchQuery%' "
                "ORDER BY $kColumnEmpName ASC";
            ...
            _whereClause =
                "WHERE $kColumnDepartment = '$_department' AND $kColumnEmpName LIKE '%$searchQuery%' "
                "OR $kColumnEmpNo LIKE '%$searchQuery%' "
                "OR $kColumnTelNo LIKE '%$searchQuery%' "
                "OR $kColumnMobileNo LIKE '%$searchQuery%' "
                "ORDER BY $kColumnEmpName ASC";
            ...
            _whereClause =
                "WHERE $kColumnGrade = 'E7' OR $kColumnGrade = 'E8' OR "
            ...
            _whereClause =
                "WHERE $kColumnGrade = 'E8' OR $kColumnGrade = 'E9' OR "
            ...
            _whereClause =
                "WHERE $kColumnGrade = 'E9' OR $kColumnGrade = 'A' OR $kColumnGrade = 'B' AND $kColumnEmpName LIKE '%$searchQuery%' "
                "OR $kColumnEmpNo LIKE '%$searchQuery%' "
                "OR $kColumnTelNo LIKE '%$searchQuery%' "
                "OR $kColumnMobileNo LIKE '%$searchQuery%' "
                "ORDER BY $kColumnEmpName ASC";
      ...
      final String _query =
          "SELECT * FROM $kTableEmployees $_whereClause $_sortingOrder";
      final _result = await _db!.rawQuery(_query);

      print('query for getting employees list is: $_query');
      debugPrint('result for getting employees list is: $_result');
    ...
    try {
      ...
      if (forBirthdayList) {
        ...
        if (allEmployees) {
          ...
          if (_contactListBy == kMyLocationAndDepartment) {
            _whereClause =
                "WHERE $kColumnDepartment = '$_department' AND $kColumnLocation = '$_location'";
          } else if (_contactListBy == kMyLocation) {
            ...
            _whereClause =
                "WHERE $kColumnGrade = 'E9' OR $kColumnGrade = 'A' OR $kColumnGrade = 'B'";
          }
        }
      ...
      final alpha =
          "SELECT * FROM $kTableEmployees $_whereClause  UNION SELECT * FROM $kTableEmployees "
          "WHERE $kColumnEmpName LIKE '%$_searchQuery%' "
          "OR $kColumnEmpNo LIKE '%$_searchQuery%' "
          "OR $kColumnTelNo LIKE '%$_searchQuery%' "
          "OR $kColumnMobileNo LIKE '%$_searchQuery%' "
  ...
  static Future<List<Employee>> getNewJoinedEmployeesList() async {
    ...
    DateTime firstDayCurrentMonth = DateTime.now().add(Duration(days: -30));

    DateTime lastDayCurrentMonth = DateTime.now();

    String FD = DateFormat("dd.MM.yyyy")
    ...
    print(
        "last day of the current month ::${DateFormat("dd.MM.yyyy").format(firstDayCurrentMonth)} $firstDayCurrentMonth $lastDayCurrentMonth");
    ...
    try {
      if (_result.isNotEmpty) {
        ...
        final List<Employee> _employeeList = _result
            ...
            .toList();
        print(
            "_employeeList date od joining :: ${_employeeList[0].DateOfJoining}");
        for (int i = 0; i < _employeeList.length; i++) {
          DateTime d =
              DateFormat("dd.MM.yyyy").parse(_employeeList[i].DateOfJoining!);

          if (d.isBefore(lastDayCurrentMonth) &&
              d.isAfter(firstDayCurrentMonth)) {
      ...
      print("length of new joined employess from DB :: ${e}");
      handleException(
        exception: e,
        stackTrace: s,
        exceptionClass: EmployeeDb().tag,
  ...
  static batchInsertIntoEmployeesTable(List<Employee> _employeeList) async {
    try {
      ...
      for (final Employee _employee in _employeeList) {
        _batch.insert(kTableEmployees, _employee.toMap(),
  ...
  static batchInsertIntoSectionTable(List<Section> _sectionList) async {
    try {
      ...
      for (final Section _employee in _sectionList) {
        _batch.insert(kTableSectionMaster, _employee.toMap(),
            conflictAlgorithm: ConflictAlgorithm.replace);
  ...
  static Future<List<String>> getDirectorateFromDepartment(
      String department, String location, String grade, String section) async {
    print(
        "getSubFunction directorate department $department location $location grade $grade");
    final _db = await DbProvider.db.database;
    // String setSection = "";
    if (section != '') {
      String findSection =
    ...
    if (department != '' &&
        ...
        section.isNullOrEmpty) {
      whereClause =
          "WHERE $kColumnDepartment == '$department' AND $kColumnLocation == '$location'";
    }
    if (department != '' &&
        location.isNullOrEmpty &&
        grade != '' &&
      ...
      if (grade == 'Executives') {
        whereClause =
            "WHERE $kColumnDepartment == '$department' AND $kColumnGrade between 'E0' and 'E9'";
      } else if (grade == 'Non-Executives') {
        whereClause =
            "WHERE $kColumnDepartment == '$department' AND $kColumnGrade between 'S0' and 'S7'";
      } else {
        whereClause =
            "WHERE $kColumnDepartment == '$department' AND $kColumnGrade == '$grade'";
      }
    }
    if (department.isNullOrEmpty &&
        location.isNullOrEmpty &&
    ...
    if (department.isNullOrEmpty &&
        ...
        section != '') {
      whereClause =
          "WHERE $kColumnSection = '$section' AND $kColumnLocation = '$location'";
    }
    if (department.isNullOrEmpty &&
        location != '' &&
        grade.isNullOrEmpty &&
    ...
    if (department.isNullOrEmpty &&
      ...
      if (grade == 'Executives') {
        ...
        whereClause =
            "WHERE $kColumnSection = '$section' AND $kColumnGrade = '$grade' AND $kColumnLocation = '$location' ";
      }
    }
    if (department.isNullOrEmpty &&
        location.isNullOrEmpty &&
      ...
      if (grade == 'Executives') {
        ...
        whereClause =
            "WHERE $kColumnSection = '$section' AND $kColumnGrade = '$grade'";
      }
    }
    if (department != '' &&
        location != '' &&
      ...
      if (grade == 'Executives') {
        ...
        whereClause =
            "WHERE $kColumnDepartment = '$department' AND $kColumnGrade = '$grade' AND $kColumnLocation = '$location'";
      }
    }

    String _query =
        "select distinct($kColumnSDirectorate) from $kTableEmployees $whereClause ORDER BY $kColumnSDirectorate ASC";
    print("query to find directorate :: $_query");
    final _result = await _db!.rawQuery(_query);
    print("query to find directorate :: $_result");
    if (_result.isNotEmpty) {
      final List<String> subDepartmentList =
          _result.map<String>((s) => s['Directorate'].toString()).toList();
  ...
  static Future<List<String>> getEmployeeSection(String department,
      String location, String grade, String directorate) async {
    final _db = await DbProvider.db.database;
    ...
    if (department.isNullOrEmpty &&
        ...
        directorate != '') {
      whereClause =
          "WHERE $kColumnSDirectorate = '$directorate' AND $kColumnLocation = '$location'";
    }
    if (department.isNullOrEmpty &&
        location != '' &&
        grade.isNullOrEmpty &&
    ...
    if (department.isNullOrEmpty &&
      ...
      if (grade == 'Executives') {
        ...
        whereClause =
            "WHERE $kColumnSDirectorate = '$directorate' AND $kColumnGrade = '$grade' AND $kColumnLocation = '$location' ";
      }
    }
    if (department.isNullOrEmpty &&
        location.isNullOrEmpty &&
      ...
      if (grade == 'Executives') {
        ...
        whereClause =
            "WHERE $kColumnSDirectorate = '$directorate' AND $kColumnGrade = '$grade'";
      }
    }
    if (department != '' &&
        location != '' &&
    ...
    String _query =
        "select distinct($kColumnSection) from $kTableEmployees $whereClause ORDER BY $kColumnSection ASC";
    print("_query to find section in employess :: $_query");
    final _result = await _db!.rawQuery(_query);
    print("_query to find section in employess :: $_result");
    if (_result.isNotEmpty) {
      final List<String> employeeSection =
          _result.map<String>((s) => s['Section'].toString()).toList();
      ...
      for (int i = 0; i < employeeSection.length; i++) {
        if (employeeSection[i] == "") {
          ...
          String q =
              "select $kSectionDes from $kTableSectionMaster where $kSection = '${employeeSection[i]}'";
          final _result = await _db.rawQuery(q);
          final _result1 = await _db.rawQuery(q1);
          final employeeSectionDes =
              _result.map<String>((s) => s[kSectionDes].toString());

          sectionDes.add(employeeSectionDes.first);

          print(
              "Description from abbribation :: ${_result}  ${employeeSectionDes}  ${_result1}");
        }
      }
