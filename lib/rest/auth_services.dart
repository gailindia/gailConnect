...
class AuthServices extends ApiServices {
    ...
    try {
      ...
      if (_response.statusCode == 200 && _response.body.isNotEmpty) {
        ...
        if (_responseVal) {
          ...
          pref.putString("baName", _responseBody[kJsonBaName],
              isEncrypted: true);
          pref.putString("gstInNumber", _responseBody[kJsonGstIn],
              isEncrypted: true);
          pref.putString("cpfNumber", _responseBody[kJsonCpfNumber],
              isEncrypted: true);
          pref.putString("userAccess", _responseBody[kJsonUserAccess],
              isEncrypted: true);
          pref.putString("gstLocation", _responseBody[kJsonGstLocation],
              isEncrypted: true);
          pref.putString("businessArea", _responseBody[kJsonBusinessArea],
              isEncrypted: true);
          pref.putString("gstLocation", _responseBody[kJsonGstLocation],
              isEncrypted: true);

          pref.putString("isFcmToken", _responseBody[kJsonToken],
              isEncrypted: true);
          pref.putString("authtoken", _responseBody['token'],
              isEncrypted: true);

          if (Platform.isAndroid) {
            // _sharedPrefs.apkVersion = _responseBody[kJsonApkVersionNo];
            pref.putString("apkVersion", _responseBody[kJsonApkVersionNo],
                isEncrypted: true);
          } else if (Platform.isIOS) {
            pref.putString("iosVersion", _responseBody[kJsonIpaVersion],
                isEncrypted: true);
            // _sharedPrefs.iosVersion = _responseBody[kJsonIpaVersion];
          }

          bool? isEmployeeDataSyncedAfterAlter =
              await pref.getBool("isEmployeeDataSyncedAfterAlter") ?? false;
          print(
              "isEmployeeDataSyncedAfterAlter :: $isEmployeeDataSyncedAfterAlter");
          ...
          String _result = await _isDashboardAdmin(cpfNumber: cpf!);
  ...
  Future<String> _isDashboardAdmin({required String cpfNumber}) async {
    ...
    try {
      ...
      final _response = await httpClient.get('$kIsDashboardAdminApi$cpfNumber',
          headers: headerData);
