...
class _StackedCardCarouselState extends State<StackedCardCarousel> {
  ...
  Widget build(BuildContext context) {
    widget._pageController.addListener(() {
      if (mounted) {
        setState(() => _pageValue = widget._pageController.page!);
      }
    });
      ...
      (MapEntry<int, Widget> item) {
        ...
        switch (widget._type) {
          case StackedCardCarouselType.fadeOutStack:
            ...
            return Positioned.fill(
              ...
              child: Align(
                ...
                child: Wrap(
                  children: <Widget>[
                    Transform.scale(scale: scale, child: Opacity(opacity: opacity, child: item.value)),
                  ],
                ),
              ),
            );
          ...
          default:
            ...
            return Positioned.fill(
              top: -position + (10.0 * item.key),
              child: Align(
                alignment: Alignment.topCenter,
                child: Wrap(children: <Widget>[Transform.scale(scale: scale, child: item.value)]),
              ),
            );
        }
      },
...
class ClickThroughStack extends Stack {
  ClickThroughStack({Key? key, required List<Widget> children}) : super(key: key, children: children);
...
class ClickThroughRenderStack extends RenderStack {
  ClickThroughRenderStack({TextDirection? textDirection, required StackFit fit, required AlignmentGeometry alignment})
      : super(fit: fit, alignment: alignment, textDirection: textDirection);
